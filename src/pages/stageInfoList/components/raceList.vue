    ...
    <div>
        <div class="chart wid100p h426 mt44 pl35 pr35 pt15 fsize12 rel">
            <div class="flex h36 fsize14 col29564c">
                <p class="wid111 pl25">学生姓名</p>
                <p class="wid230">获奖名称</p>
                <p class="wid170">级别</p>
                <p class="wid85">等级</p>
                <p class="wid190">获奖时间</p>
                <!-- <p class="wid170">备注</p> -->
                <p class="wid130 center">操作</p>
            </div>
            <div v-if="list.length>0">
                <div class="table flex fsize12 h56 mb5" v-for="(item,index) in list" :key="index">
                    <div class="wid111 pl25 ellipsis2">{{item.stuName}}</div>
                    <div class="wid230 ellipsis2">{{item.name}}</div>
                    <div class="wid170">{{item.level}}</div>
                    <div class="wid85">{{item.grade}}</div>
                    <div class="wid190 ellipsis2">{{item.awardTime}}</div>
                    <!-- <div class="wid170 ellipsis2">{{item.remark}}</div> -->
                    <div class="operation wid130 flex-center cursor">
                        <router-link tag="p" :to="{path:'/stageAddRace',query:{type:1,handle:2,id:item.id}}" class="mr10">查看</router-link>
                        <router-link tag="p" :to="{path:'/stageAddRace',query:{type:1,handle:3,id:item.id}}" class="mr10">修改</router-link>
            ...
            <!-- 按钮 -->
            <router-link tag="div" :to="{path:'/stageAddRace',query:{type:1,handle:1}}" class="add wid100 lh33 fsize14 center abs l0 t-33 cursor">添加</router-link>
            <div class="dao wid100 lh33 fsize14 col29564c center abs l110 t-33 cursor rel">批量导入
    ...
    methods:{
        ...
        Remove(id){
            this.$confirm('是否删除此条记录?', '提示').then(() => {
                this.$fetch({
                    url:'api/competitionAward/delete',
                    type:'delete',
                    data:{id}
                }).then(res=>{
        ...
        dTemplate(){
            ...
            a.href = window.hez_baseURL+'api/downLoad/template?type=2';
