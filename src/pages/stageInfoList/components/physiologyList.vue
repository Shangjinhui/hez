    ...
    <div>
        <div class="chart wid100p h426 mt44 pl35 pr35 pt15 fsize12 center rel">
            <div class="flex h36 fsize14 col29564c">
                ...
                <p class="flex-son">身高</p>
                <p class="flex-son">体重</p>
                <p class="flex-son">肺活量</p>
                <p class="flex-son">左眼视力</p>
                <p class="flex-son">右眼视力</p>
            ...
            <div v-if="list.length>0">
                <div class="table flex fsize12 h56 mb5" v-for="(item,index) in list" :key="index">
                    ...
                    <div class="flex-son ellipsis2">{{item.testTime}}</div>
                    <div class="flex-son ellipsis2">{{item.height}}</div>
                    <div class="flex-son ellipsis2">{{item.weight}}</div>
                    <div class="flex-son ellipsis2">{{item.vitalCapacity}}</div>
                    <div class="flex-son ellipsis2">{{item.visionLeft}}</div>
                    <div class="flex-son ellipsis2">{{item.visionRight}}</div>
                    <div class="operation wid130 flex-center cursor">
                        <router-link tag="p" :to="{path:'/stageAddPhy',query:{handle:3,id:item.id}}" class="mr10">修改</router-link>
...
export default {
    ...
    methods:{
        getData(){
            this.$fetch({
                url:'api/sportsHealth/getBodyHealthByClass',
                data:{
                    page:this.page,
                    limit:this.limit,
        ...
        initBatch(){
            ...
            this.$refs['batch'].addEventListener('change',function(e){
                ...
                that.$fetch({
                    url:'api/sportsHealth/BodyImportExcel',
                    type:'post',
                    data:fromData
                }).then(res=>{
                    e.target.value = '';
        ...
        dTemplate(){
            ...
            a.href = window.hez_baseURL+'api/downLoad/template?type=6';
